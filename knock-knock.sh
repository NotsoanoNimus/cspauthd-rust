...
if [[ -n "${2//[0-9]/}" || -n "${3//[0-9]/}" ]]; then
    echo "ERROR: 'srcIP' or 'timer' contains invalid integer value."
    exit 1
fi

if [[ -n "$(echo "$SRCIP" | grep -P '^([0-9]{1,3}\.){3}[0-9]{1,3}$')" ]]; then
    FAM="4"
elif [[ -z "${SRCIP//[0-9a-fA-F:]/}" ]]; then
    FAM="6"
else
    echo "ERROR: Invalid address family."
    exit 1
fi
